...
- NAO_CODEX_EXTRA_RO_BIND: colon-separated host paths to expose read-only
- NAO_CODEX_EXTRA_RW_BIND: colon-separated host paths to expose read-write
EOF
  exit 0
fi
...
if [[ -n "${NAO_CODEX_EXTRA_RO_BIND:-}" ]]; then
  add_colon_separated_binds "ro" "$NAO_CODEX_EXTRA_RO_BIND"
fi

if [[ -n "${NAO_CODEX_EXTRA_RW_BIND:-}" ]]; then
  add_colon_separated_binds "rw" "$NAO_CODEX_EXTRA_RW_BIND"
fi
